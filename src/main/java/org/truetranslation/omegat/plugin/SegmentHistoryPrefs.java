...
public class SegmentHistoryPrefs extends BasePreferencesController {
    ...
    private static final String DEFAULT_TEMPLATE = "${hour}:${minute}:${second} ${day}/${month} (${length} chars): ${text}";
    ...
    public SegmentHistoryPrefs() {
        ...
        // Limit height so it doesn't stretch vertically
        ...
        // 5. Shortcut (OmegaT Style: BoxLayout.LINE_AXIS)
    ...
    // Renamed static accessors to be clear, though no longer conflicting with JPanel methods
    public static boolean isHistoryEnabled() {
        return Boolean.parseBoolean(Preferences.getPreferenceDefault(PREF_ENABLED, "true"));
    }
    
    // Alias for compatibility if needed, or use isHistoryEnabled
